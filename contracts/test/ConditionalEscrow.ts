...
describe("Social Features", function () {
  // Constants (duplicated for this describe block scope)
  const AMOUNT = ethers.parseUnits("100", 6);
  const MARKET_ID = "0xabc123def456";

  // Reuse constants and fixture from outer describe block
  async function deployEscrowFixture() {
    const [owner, resolver, depositor, beneficiary, otherUser] = await ethers.getSigners();
    const MockUSDC = await ethers.getContractFactory("MockUSDC");
    const usdc = await MockUSDC.deploy() as unknown as MockUSDC;
    ...
    await usdc.mint(depositor.address, AMOUNT * 100n);
    return { escrow, usdc, owner, resolver, depositor, beneficiary, otherUser };
  }

  it("Should allow setting username", async function () {
    const { escrow, depositor } = await loadFixture(deployEscrowFixture);
    
    await escrow.connect(depositor).setUsername("alice");
    
    expect(await escrow.getUsername(depositor.address)).to.equal("alice");
  });
  ...
  it("Should reject empty username", async function () {
    ...
    await expect(
      escrow.connect(depositor).setUsername("")
    ).to.be.revertedWithCustomError(escrow, "UsernameEmpty");
  });

  it("Should reject username over 32 chars", async function () {
    const { escrow, depositor } = await loadFixture(deployEscrowFixture);
    
    const longName = "a".repeat(33);
    await expect(
      escrow.connect(depositor).setUsername(longName)
    ).to.be.revertedWithCustomError(escrow, "UsernameTooLong");
  });

  it("Should track user stats after resolution", async function () {
    const { escrow, usdc, resolver, depositor, beneficiary } = await loadFixture(deployEscrowFixture);
    
    // Create and resolve escrow
    await usdc.connect(depositor).approve(await escrow.getAddress(), AMOUNT);
    await escrow.connect(depositor).createEscrow(beneficiary.address, AMOUNT, MARKET_ID, true);
    await escrow.connect(resolver).resolveEscrow(0, true); // Beneficiary wins
    
    // Check stats
    const beneficiaryStats = await escrow.getUserStats(beneficiary.address);
    expect(beneficiaryStats.totalWon).to.equal(AMOUNT);
    expect(beneficiaryStats.escrowsWon).to.equal(1);
    
    const depositorStats = await escrow.getUserStats(depositor.address);
    expect(depositorStats.totalLost).to.equal(AMOUNT);
    expect(depositorStats.escrowsLost).to.equal(1);
    expect(depositorStats.escrowsCreated).to.equal(1);
  });
});